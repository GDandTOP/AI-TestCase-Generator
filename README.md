...
# 🧪 TestPlanner (AI Test Case Generator)
...
<p align="center">
  <img src="https://img.shields.io/badge/버전-1.0.0-blue?style=flat-square" alt="버전" />
  <img src="https://img.shields.io/badge/라이선스-MIT-green?style=flat-square" alt="라이선스" />
...
**TestPlanner**는 개발자가 코드를 변경한 뒤, QA 팀이 **어떤 테스트를 해야 할지** 자동으로 만들어 주는 도구입니다.

로컬 Git 저장소 경로 또는 GitHub URL을 입력하면, 두 브랜치(또는 커밋) 사이의 **변경된 코드(diff)** 를 추출하고, **Claude AI**가 이를 분석해 **마크다운 형식의 QA 테스트케이스 문서**를 실시간으로 생성합니다.
...
| | TypeScript | 5 | 타입 안전성 |
| | simple-git | 3.x | Git diff 추출 |
| | Zod | 3.x | 환경변수 검증 |
| | marked | 17 | 마크다운 파싱 |
| | Puppeteer | 24 | PDF 생성 |
| **AI** | Claude API | @anthropic-ai/sdk 0.x | 영향도 분석 / TC 생성 |
| | SSE | - | 실시간 스트리밍 |
| **기타** | npm workspaces | - | 모노레포 관리 (server + client) |
| | concurrently | 8.x | 서버·클라이언트 동시 실행 |
...
```bash
node --version   # v18 이상 권장 (v20 사용 권장)
npm --version    # v9 이상
git --version    # 아무 버전이나 OK
```

**Anthropic API 키**가 필요합니다. [console.anthropic.com](https://console.anthropic.com)에서 발급받으세요.
...
```bash
# 1. 저장소 클론 (로컬 폴더명은 cicd_test_maker 등 자유롭게 사용 가능)
git clone https://github.com/GDandTOP/AI-TestCase-Generator.git
cd AI-TestCase-Generator

# 2. 전체 의존성 설치 (server + client 동시, npm workspaces)
npm install
```
...
`server/.env` 파일을 열어 **Anthropic API 키**를 입력하세요.
...
> ⚠️ `server/.env`는 **절대 Git에 커밋하지 마세요.** `.gitignore`에 포함되어 있습니다.
...
로컬 워크스페이스 폴더명은 `cicd_test_maker` 등 무엇이든 사용 가능하며, 저장소 클론 시 기본 폴더명은 `AI-TestCase-Generator`입니다.
...
│       │   ├── file.service.ts          # MD/PDF 파일 저장
│       │   └── project-context.service.ts  # 저장소 내 문서·설정 수집
│       ├── utils/
│       │   ├── prompt.util.ts       # Claude 프롬프트 빌더
│       │   ├── markdown.util.ts     # MD 리포트 포맷터
│       │   ├── pdf.util.ts          # PDF 생성 (Puppeteer)
│       │   ├── log-payload.util.ts  # 디버그용 요청 payload 로깅
│       │   └── claude-error.util.ts # Claude API 에러 처리
│       └── types/
...
└── logs/                       # 디버그용 API 요청 payload (선택 사용)
```
...
| `POST` | `/api/git/project-context` | 저장소 내 주요 문서·설정 수집 | `{ repoPath: string }` |
| `POST` | `/api/git/diff` | Git diff 추출 | `{ repoPath, base, head }` |
| `POST` | `/api/git/clone` | GitHub URL에서 저장소 클론 | `{ githubUrl: string }` |
| `GET` | `/api/git/open-folder` | macOS 폴더 선택 다이얼로그 실행 후 선택 경로 반환 | - |

### 분석 및 테스트케이스

| Method | Endpoint | 설명 | 요청 Body |
|--------|----------|------|-----------|
| `POST` | `/api/analysis/impact` | Claude AI 영향도 분석 | `{ diff, model?, projectContext? }` |
...
| `POST` | `/api/testcase/save-pdf` | PDF 파일 저장 | `{ content, filename? }` |
| `POST` | `/api/testcase/pdf-download` | PDF 생성 후 다운로드용 URL 반환 | `{ content, filename? }` |
| `GET` | `/api/testcase/list` | 저장된 테스트케이스 파일 목록 | - |
...
| `OUTPUT_DIR` | 선택 | `../output` | MD/PDF 파일 저장 디렉토리 경로 (프로젝트 루트 기준) |
| `MAX_DIFF_SIZE` | 선택 | `50000` | diff 최대 크기(바이트). 초과 시 자동 잘림 |
| `NODE_ENV` | 선택 | `development` | 실행 환경 (`development` \| `production` \| `test`) |
| `FRONTEND_URL` | 선택 | - | 프로덕션 CORS 허용 출처 (미설정 시 `http://localhost:5173`) |
...
- **기본 모델**: `claude-haiku-4-5-20251001` (영향도 분석·TC 생성 모두 요청 시 `model` 파라미터로 변경 가능)
...
로컬 Git 저장소 경로를 직접 입력하거나, 폴더 선택 버튼(`/api/git/open-folder`)을 클릭하거나, GitHub URL을 붙여넣은 뒤 클론합니다. 저장소가 로드되면 비교할 **base** 브랜치와 **head** 브랜치를 선택합니다.
...
「diff 추출」으로 변경된 파일 목록을 불러온 뒤, 「AI 분석 시작」을 클릭합니다. Claude AI가 변경 내용의 **기능적 영향 범위**를 JSON으로 파싱해 표시합니다. (선택) 프로젝트 컨텍스트를 불러와 분석 품질을 높일 수 있습니다.

**Step 3 — 테스트케이스 생성**

「TC 생성」을 누르면 SSE 스트리밍으로 마크다운 테스트케이스가 실시간으로 화면에 타이핑됩니다. 완료 후 MD 파일로 저장하거나 PDF로 내보내고, 목록에서 다운로드할 수 있습니다.
...
# 응답 예: {"status":"ok","timestamp":"2026-03-09T00:00:00.000Z"}
```

### TypeScript 타입 체크

서버·클라이언트 각각 타입 체크 스크립트가 있다면:
...
- **`server/.env`** 는 절대 Git에 커밋하지 마세요. `.gitignore`에 포함되어 있습니다.
- **`output/`** 디렉토리는 서버가 파일 저장 시 자동으로 생성됩니다.
- **`MAX_DIFF_SIZE`** 를 초과하면 diff가 잘려서 분석되므로, 대규모 PR은 범위를 나눠 분석하는 것을 권장합니다.
- GitHub URL 클론은 서버의 임시 디렉토리에 저장되며, 서버 재시작 시 삭제될 수 있습니다.
- SSE 스트리밍 중 오류 시 `data: {"type": "error", "message": "..."}` 이벤트로 전달됩니다.
...
<div align="center">
  <sub>Made with ❤️ using Claude AI · TestPlanner (AI Test Case Generator) v1.0.0</sub>
</div>
